...
class NiceCXBuilder(object):
    ...
    def add_network_attribute(self, name=None, values=None, attribute_type=None, cx_element=None):
        """
        ...
        if attribute_type:
            add_this_network_attribute['d'] = attribute_type

        self.network_attribute_inventory[name] = add_this_network_attribute

    def add_node(self, name=None, represents=None, node_id=None, data_type=None, map_node_ids=False):
        """
        ...
        if self.node_inventory.get(name) is not None:
            return self.node_inventory.get(name).get('@node_id')
        ...
        add_this_node = {'@node_id': node_id, 'n': name}
        if represents:
    ...
    def add_edge(self, source=None, target=None, interaction=None, edge_id=None):
        """
        ...
        :param edge_id: Edge id for this edge.  If none is provided the builder will create one
        :type edge_id: int
        :return: Edge ID
        :rtype: int
        """
        ...
        add_this_edge = {'@edge_id': edge_id, 's': source, 't': target}
        if interaction:
    ...
    def add_node_attribute(self, property_of, name, values, attribute_type=None):
        """
        ...
        if attribute_type:
            if attribute_type == 'float' or attribute_type == 'double':
                attribute_type = 'double'
                try:
                    if not isinstance(values, float):
                        add_this_node_attribute['v'] = float(values)
                except ValueError as e:
                    raise ValueError('Value was not of attribute_type %s' % attribute_type)
            if attribute_type == 'list_of_float' or attribute_type == 'list_of_double':
                try:
            ...
            add_this_node_attribute['d'] = attribute_type
        else:
    ...
    def add_edge_attribute(self, property_of=None, name=None, values=None, attribute_type=None):
        """
        Set the value(s) of attribute of an edge, where the edge may be specified by its id or passed in an object.

        Example:

            ``set_edge_attribute(0, 'weight', 0.5, attribute_type='float')``
        ...
        if attribute_type:
            ...
            if attribute_type == 'list_of_float' or  attribute_type == 'list_of_double':
                ...
                attribute_type = 'list_of_double'
            add_this_edge_attribute['d'] = attribute_type
        else:
            use_this_value, attr_type = self._infer_data_type(values)
    ...
    def get_nice_cx(self):
        ...
        for k, v in self.network_attribute_inventory.items():
            self.nice_cx.add_network_attribute(name=v.get('n'), values=v.get('v'), attribute_type=v.get('d'))
