...
from nicecxNdex import solr_url
    ...
    def query_network_fast(self, uuid, search_string, max_edges):
        ...
        try:
            ...
            if('404' in se.message):
                nicecxNdex.get_logger('SOLR').warning('Network not found ' + self.uuid + ' on ' + solr_url + ' server.')
                raise Exception("Network not found (SOLR)")
            else:
                nicecxNdex.get_logger('SOLR').warning('Network error ' + self.uuid + ' on ' + solr_url + ' server. ' + se.message)
                raise Exception(se.message)
        except StopIteration as si:
                nicecxNdex.get_logger('QUERY').warning("Found more than max edges.  Raising exception")
